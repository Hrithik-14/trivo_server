...
export const registerUser = async ( req: Request, res: Response, next: NextFunction ) => {
    ...
    const employeeCode = `TR/${new Date().getFullYear()}/${allUsers.toString().padStart(2, "0")}`;
...
export const getAllManagersDetail = async ( req: Request, res: Response, next: NextFunction ) => {
    const managers = await User.find({ role: 'manager' }, { _id: 1, name: 1, employeeCode: 1 })
...
export const getmanagersEmployees = async ( req: Request, res: Response, next: NextFunction ) => {
    const page = parseInt(req.query.page as string) || 1;
    const limit = parseInt(req.query.limit as string) || 10;
    const skip = (page - 1) * limit;
    const managerId = req.user?.id

    const total = await User.countDocuments({ role: 'employee', managerId: managerId })
    const managers = await User.find({ managerId: managerId }).sort({ createdAt: -1, _id: -1 }).skip(skip).limit(limit)
    res.status(200).json({ total, page, totalPages: Math.ceil(total / limit), managers })
}
